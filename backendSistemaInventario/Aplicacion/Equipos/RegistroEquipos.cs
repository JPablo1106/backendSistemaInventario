...
namespace backendSistemaInventario.Aplicacion.Equipos
{
    ...
    {
        public class EjecutarRegistroEquipo : IRequest<EquipoDTO>
        {
        ...
        public class EjecutaValidacion : AbstractValidator<EjecutarRegistroEquipo>
        {
            public EjecutaValidacion()
            {
                ...
                RuleFor(p => p.velocidadProcesador).NotEmpty().WithMessage("La velocidad del procesador del equipo es requerido");
                RuleFor(p => p.tipoProcesador).NotEmpty().WithMessage("El tipo de procesador del equipo es requerido");
                RuleFor(p => p.memoriaRam).NotEmpty().WithMessage("La memoria ram del equipo es requerida");
                RuleFor(p => p.tipoMemoriaRam).NotEmpty().WithMessage("El tipo de memoria ram del equipo es requerido");

                //Validaciones del Disco Duro
                RuleFor(p => p.marcaDisco).NotEmpty().WithMessage("La marca del disco duro es requerida");
                RuleFor(p => p.modeloDisco).NotEmpty().WithMessage("El modelo del disco duro es requerida");
                RuleFor(p => p.c).GreaterThanOrEqualTo(0).WithMessage("La particion C no puede ser negativa");

            }
        }

        public class Manejador : IRequestHandler<EjecutarRegistroEquipo, EquipoDTO>
        {
            ...
            public async Task<EquipoDTO> Handle (EjecutarRegistroEquipo request, CancellationToken cancellationToken)
            {

                //Veridficar si el disco duro ya existe
                ...
                var nuevoEquipo = new Equipo
                {
                    ...
                    tipoProcesador = request.tipoProcesador,
                    memoriaRam = request.memoriaRam,
                    tipoMemoriaRam = request.tipoMemoriaRam,
                ...
                if(resultado > 0)
                {
                    var equipoDTO = _mapper.Map<EquipoDTO>(nuevoEquipo);
                ...
                throw new Exception("No se pudo registrar el equipo");
            }
        }
    }
}

...
{
    public class ConsultarAsignacionesMouse
    {
        public class EjecutarConsultaMouse : IRequest<List<AsignacionDTO>>
        {
        }

        public class Manejador : IRequestHandler<EjecutarConsultaMouse, List<AsignacionDTO>>
        {
            ...
            public async Task<List<AsignacionDTO>> Handle(EjecutarConsultaMouse request, CancellationToken cancellationToken)
            {
                ...
                var asignaciones = await _context.asignaciones
                    ...
                    .Where(a => a.detalleAsignaciones.Any(d => d.componente is Mouse))
                ...
                var asignacionesDto = asignaciones.Select(a => new AsignacionDTO
                {
                    ...
                    detalleAsignaciones = a.detalleAsignaciones
                        ...
                        .Select(d => new DetalleAsignacionDTO
                        {
                            ...
                            componente = d.componente is Mouse mouse ? new ComponenteDTO
                            {
                                idComponente = mouse.idComponente,
                                tipoComponente = mouse.tipoComponente,
                                marcaComponente = mouse.marcaComponente,
                            } : null
                        }).ToList()
                }).ToList();

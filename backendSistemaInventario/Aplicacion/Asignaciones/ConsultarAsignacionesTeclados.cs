...
{
    public class ConsultarAsignacionesTeclados
    {
        public class EjecutarConsultaTeclados : IRequest<List<AsignacionDTO>>
        {
        }

        public class Manejador : IRequestHandler<EjecutarConsultaTeclados, List<AsignacionDTO>>
        {
            ...
            public async Task<List<AsignacionDTO>> Handle(EjecutarConsultaTeclados request, CancellationToken cancellationToken)
            {
                ...
                var asignaciones = await _context.asignaciones
                    ...
                    .Where(a => a.detalleAsignaciones.Any(d => d.componente is Teclado))
                ...
                var asignacionesDto = asignaciones.Select(a => new AsignacionDTO
                {
                    ...
                    detalleAsignaciones = a.detalleAsignaciones
                        .Where(d => d.componente is Teclado)
                        .Select(d => new DetalleAsignacionDTO
                        {
                            ...
                            componente = d.componente is Teclado teclado ? new ComponenteDTO
                            {
                                idComponente = teclado.idComponente,
                                tipoComponente = teclado.tipoComponente,
                                marcaComponente = teclado.marcaComponente,
                                idiomaTeclado = teclado.idiomaTeclado
                            } : null
                        }).ToList()
                }).ToList();
